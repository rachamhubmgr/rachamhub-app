...
export function OrderExtractionForm() {
  ...
  const extractOrderFromText = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to extract order';
      setError(message);
  ...
  return (
    <div className="space-y-6">
      {/* Input Section */}
      <Card className="p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">Paste Order Information</h2>
        ...
        <Card className="bg-green-50 border-green-200 dark:bg-green-950 dark:border-green-800 p-4 flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-green-600/80 dark:text-green-400/80">
              The order has been added to the system and is ready for processing.
            </p>
          </div>
        </Card>
      )}
      ...
      {extractedData && (
        <Card className="p-6 bg-accent/5 border-primary/20">
          <h2 className="text-lg font-semibold text-foreground mb-4">Extracted Order Details</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-foreground font-medium">{extractedData.customerName}</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">
                  Customer ID
                </label>
                <p className="text-foreground font-medium">{extractedData.customerId}</p>
              </div>
            </div>
                  ...
                  <tbody>
                    {extractedData.items.map((item, idx) => (
                      <tr key={idx} className="border-b border-border/50">
                        ...
                        <td className="py-3 text-foreground">{item.weight || '—'}</td>
                        <td className="py-3 text-foreground">{item.dimensions || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Card className="p-6 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
        ...
        <ul className="text-sm text-foreground/80 space-y-1 list-disc list-inside">
          ...
          <li>The AI will extract and structure the information automatically</li>
        </ul>
      </Card>
    </div>
  );
