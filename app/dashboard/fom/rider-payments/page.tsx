    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Calculate and review how much each rider should be paid based on deliveries and failed attempts.
          </p>
        </div>
      ...
      {loading ? (
        <Card className="p-12 text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">Loading rider data...</p>
        </Card>
      ) : riderGroups.length === 0 ? (
        <Card className="p-12 text-center">
          <Users className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
          <p className="text-muted-foreground">No rider assignments found for this date.</p>
        </Card>
      ) : (
        <>
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 flex items-center justify-between">
            <div>
              <p className="text-sm text-muted-foreground">Total Company Payout</p>
                ...
                <Card key={entry.rider} className="p-6 space-y-4">
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-muted/40">
                        <tr>
                          ...
                          <th className="text-right p-3 text-xs font-semibold text-muted-foreground">Rider Pay</th>
                        </tr>
                      </thead>
                      <tbody>
                        {entry.deliveries.map((d, i) => (
                          <tr key={d.orderId} className={`border-t border-border ${d.isFailed ? "bg-red-50" : ""}`}>
                            <td className="p-3 text-muted-foreground text-xs">{i + 1}</td>
                            <td className="p-3 font-medium">{d.customer}</td>
                            <td className="p-3 text-muted-foreground text-xs">{d.landmark}</td>
                            <td className="p-3">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase ${d.isFailed ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>
                                {d.isFailed ? "Failed" : d.status}
                              </span>
                            </td>
                            <td className="p-3 text-right font-semibold">{formatCurrency(d.riderPay)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="rounded-xl bg-muted/20 p-4 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Subtotal ({entry.deliveries.length} deliveries)</span>
                      <span className="font-semibold">{formatCurrency(entry.totalPay)}</span>
                    </div>
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => updateOutstanding(entry.rider, e.target.value)}
                        />
                      </div>
                    </div>
